...
CREATE TABLE `Users`
(
    `user_id`       BIGINT              NOT NULL AUTO_INCREMENT COMMENT '회원 고유 식별 ID / AUTO_INCREMENT 사용',
...
(
    `product_id`          BIGINT              NOT NULL AUTO_INCREMENT COMMENT '제품 고유 식별 ID / AUTO_INCREMENT 사용',
    ...
    `product_price`       DECIMAL(10, 2)      NOT NULL COMMENT '100원 <= 제품 가격',
    `product_stock`       INT                 NOT NULL DEFAULT 0 COMMENT '재고 수량, 음수 불가',
...
CREATE TABLE `Orders`
(
    `order_id`           BIGINT              NOT NULL AUTO_INCREMENT COMMENT '주문 고유 식별 ID / AUTO_INCREMENT 사용',
    `user_id`            BIGINT              NOT NULL COMMENT '주문 회원 고유 식별 ID, 외래키',
    `order_number`       VARCHAR(255) UNIQUE NOT NULL COMMENT '주문번호, 유니크 속성',
    `order_total_amount` INT                 NOT NULl COMMENT '전체 주문 수량, 음수 불가',
    `order_total_price`  DECIMAL(10, 2)      NOT NULL COMMENT '전체 주문 금액, 음수 불가',
...
(
    `order_product_id`          BIGINT         NOT NULL AUTO_INCREMENT COMMENT '주문 상품 고유 식별 ID / AUTO_INCREMENT 사용',
    `order_id`                  BIGINT         NOT NULL COMMENT '주문 고유 식별 ID, 외래키',
    `product_id`                BIGINT         NOT NULL COMMENT '제품 고유 식별 ID, 외래키',
    `order_product_amount`      INT            NOT NULL COMMENT '제품 수량, 음수 불가',
    `order_product_price`       DECIMAL(10, 2) NOT NULL COMMENT '제품 가격, 음수 불가',
    `order_product_total_price` DECIMAL(10, 2) NOT NULL COMMENT '제품 가격 총합, 음수 불가',
    `created_date`              DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일자 / 수정 불가',
    `modified_date`             DATETIME       NOT NULL COMMENT '수정일자',
...
(
    `payment_id`          BIGINT         NOT NULL AUTO_INCREMENT COMMENT '결제 고유 식별 ID / AUTO_INCREMENT 사용',
    `order_id`            BIGINT         NOT NULL COMMENT '주문 고유 식별 ID, 외래키',
    `user_id`             BIGINT         NOT NULL COMMENT '회원 고유 식별 ID, 외래키',
    `payment_method`      VARCHAR(255)   NOT NULL COMMENT '결제 수단: [CREDIT_CARD]',
    `payment_paid_amount` DECIMAL(10, 2) NOT NULL COMMENT '결제 금액, 음수 불가',
